    ...
    function __construct($auth0_service,$login_logo_url)
    {
        ...
        add_action(
            'login_enqueue_scripts', function () {
                wp_enqueue_script('simple-auth0-login-modal',  plugins_url('includes/modal/modal.js', dirname(__FILE__)));
                wp_enqueue_style('simple-auth0-login-modal',  plugins_url('includes/modal/modal.css', dirname(__FILE__)));
